...
def convert_columns_datetime(df, columns):
    for column in columns:
        df[column] = pd.to_datetime(df[column])
            
def convert_columns_bool(df, columns):
    for column in columns:
...
def create_df():
    ...
    convert_columns_datetime(df, date_columns)
    ...
    convert_columns_bool(df, bool_columns)
    return df

def refresh_data():
    st.session_state.vendor_df = create_df()
...
st.button("🔄 Refresh from Database", on_click = refresh_data)

edited_df = st.data_editor(
    st.session_state.vendor_df.copy(),
    key="vendor_data_editor",
    column_config = {
